...
def view_attribution(
    ...
    *,
    ...
    support = sum(1 for e in edges if e.edge_type is EdgeType.SUPPORTS and e.target_id == decision.id)
    contra = sum(1 for e in edges if e.edge_type is EdgeType.CONTRADICTS and e.target_id == decision.id)
    cand_for = sum(1 for e in edges if e.edge_type is EdgeType.CANDIDATE_FOR and e.target_id == decision.id)
    reviewed = sum(1 for e in edges if e.edge_type is EdgeType.REVIEWED_BY and e.source_id == decision.id)
...
def admit_attribution(view: AttributionView) -> tuple[AdmissionIssue, ...]:
    ...
    if payload.state is DecisionState.ATTRIBUTED:
        if payload.selected_candidate_id is None:
            add("attribution.selected_required", "ATTRIBUTED requires exactly one selected candidate")
        if view.support_count < 1:
            add("attribution.evidence_required", "ATTRIBUTED requires at least one supporting evidence edge")
        if payload.confidence_bp is not None and view.support_count < 1:
            add("attribution.confidence_not_admission", "confidence cannot admit ATTRIBUTED without evidence")
        if (
            payload.selected_candidate_id is not None
            and payload.selected_candidate_id not in view.approved_candidate_ids
        ):
            add("attribution.candidate_set", "selected candidate is not in the approved candidate set")
    elif payload.state is DecisionState.UNRESOLVED:
        if payload.selected_candidate_id is not None:
            add("attribution.unresolved_identity", "UNRESOLVED must not contain a selected identity")
    elif payload.state is DecisionState.CONTRADICTED:
        if view.contradict_count < 1:
            add("attribution.contradiction_required", "CONTRADICTED requires contradictory evidence")
        ...
        if payload.reason_code not in (ReasonCode.REJECTED_BY_POLICY, ReasonCode.OUT_OF_CANDIDATE_SET, ReasonCode.VALIDATION_FAILED):
            add("attribution.rejected_reason", "REJECTED requires a typed rejection reason")

    if payload.selected_candidate_id is not None and payload.selected_candidate_id not in view.approved_candidate_ids:
        if payload.state is not DecisionState.ATTRIBUTED:
            add("attribution.candidate_set", "selected candidate is not in the approved candidate set")
...
# Typing aliases without importing Sequence from typing at runtime in a circular way.
MappingLike = tuple[GraphNode, ...] | list[GraphNode]
SequenceLike = tuple[GraphEdge, ...] | list[GraphEdge]
